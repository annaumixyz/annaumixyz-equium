...
#
# Prereqs:
#   - Deployer keypair AgbSti5...ANX.json funded with ~5 SOL on devnet
#     (https://faucet.solana.com/ — paste the deployer pubkey)
#   - Program keypair ZKGM...ANX.json present (gives us the vanity program ID)
#   - `anchor build` has produced target/deploy/annaumixyz.so and target/idl/annaumixyz.json
#
# Steps:
#   1. Deploy programs/annaumixyz to devnet (bpf upgradeable, deployer = upgrade authority)
#   2. Pre-mint 21M ANX on a fresh mint, deployer ATA receives them
...
DEPLOYER_KP="$(pwd)/AgbSti5LyTfYHVytBhNP8HHz3Ko7bZfSvnsm9cJAANX.json"
PROGRAM_KP="$(pwd)/ZKGMUfxiRCXFPnqz9zgqAnuqJy15jk7fKbR4o6FuANX.json"
...
# Anchor's `target/deploy/annaumixyz-keypair.json` is overwritten by our keypair.
cp "$PROGRAM_KP" target/deploy/annaumixyz-keypair.json
solana program deploy target/deploy/annaumixyz.so \
  --program-id target/deploy/annaumixyz-keypair.json \
  --keypair "$DEPLOYER_KP" \
  --url "$RPC"
echo
...
echo "  ./target/release/annaumixyz-miner --rpc-url $RPC --keypair <miner-keypair>"
