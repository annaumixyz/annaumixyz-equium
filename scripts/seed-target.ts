...
//   ANCHOR_WALLET=./AgbSti5...ANX.json \
...
async function main() {
  ...
  const cfgBefore: any = await program.account.annaumixyzConfig.fetch(configPda);
  ...
  const cfgAfter: any = await program.account.annaumixyzConfig.fetch(configPda);
