//! Annaumixyz reference CLI miner.
//!
...
//!
//! Usage:
//!   annaumixyz-miner --rpc-url http://127.0.0.1:8899 \
...
use annaumixyz::state::{AnnaumixyzConfig, CONFIG_SEED, VAULT_SEED};
use rand::RngCore;
...
#[derive(Parser, Debug)]
#[command(version, about = "Annaumixyz reference CPU miner")]
struct Args {
    ...
    /// Override the program ID. Defaults to the value compiled into `annaumixyz`.
    #[arg(long)]
...
fn fetch_config(rpc: &RpcClient, config_pda: &Pubkey) -> Result<AnnaumixyzConfig> {
    let acct = rpc.get_account(config_pda)?;
    let mut data = acct.data.as_slice();
    let cfg = AnnaumixyzConfig::try_deserialize(&mut data)?;
    Ok(cfg)
}
...
fn submit_mine(
    ...
    let miner = miner_kp.pubkey();
    let accounts = annaumixyz::accounts::Mine {
        miner,
        config: *config_pda,
        mint: cfg.mint,
    ...
    let accounts: Vec<AccountMeta> = accounts
        ...
        .collect();
    let data = annaumixyz::instruction::Mine {
        nonce: *nonce,
        soln_indices,
    }
    .data();
...
fn submit_advance_empty_round(
    ...
    let accounts = annaumixyz::accounts::AdvanceEmptyRound {
        caller,
        config: *config_pda,
        slot_hashes: sysvar::slot_hashes::ID,
    }
    ...
    let accounts: Vec<AccountMeta> = accounts
        ...
        .collect();
    let data = annaumixyz::instruction::AdvanceEmptyRound {}.data();
    let ix = Instruction {
        program_id: *program_id,
        accounts,
        data,
