...
export const CONFIG_SEED = new TextEncoder().encode("annaumixyz-config");
export const VAULT_SEED = new TextEncoder().encode("annaumixyz-vault");
...
export interface AnnaumixyzConfig {
  mint: PublicKey;
...
export async function fetchConfig(
  program: Program<any>
): Promise<AnnaumixyzConfig | null> {
  try {
    const raw: any = await (program.account as any).annaumixyzConfig.fetch(CONFIG_PDA);
    return {
      mint: raw.mint,
