...
export function startMiner(opts: MinerOptions): MinerHandle {
  ...
  let tryInRound = 0;
  let currentConfig: AnnaumixyzConfig | null = null;
  ...
  const dispatchTo = (slot: SolverSlot, cfg: AnnaumixyzConfig) => {
    if (stopped) return;
    slot.busy = true;
    ...
    const onMessage = async (e: MessageEvent<SolveResponse>) => {
      ...
      try {
        ...
        cb.log(
          "ok",
          `mined block ${cfg.blockHeight.toString()} (+${formatBase(cfg.currentEpochReward)} ANX)`
        );
      } catch (e: any) {
    ...
    while (!stopped) {
      try {
        ...
        if (cfg.blockHeight !== lastHeight) {
          ...
          cb.log(
            "info",
            `round #${cfg.blockHeight.toString()} opened — reward ${formatBase(cfg.currentEpochReward)} ANX`
          );
        }
 ...
 * error codes start at 6000; see `programs/annaumixyz/src/errors.rs` for the
 * full enum order. */
...
function buildInputBlock(
  ...
  out.set(new TextEncoder().encode("Annaumixyz-v1"), 0);
  out.set(challenge, 9);
  out.set(miner, 41);
