...
export function clientRpcUrl(): string {
  ...
  try {
    const override = localStorage.getItem("annaumixyz:rpc-override");
...
export interface AnnaumixyzState {
  blockHeight: number;
  miningOpen: boolean;
...
export async function fetchState(): Promise<AnnaumixyzState | null> {
  try {
    const conn = readConnection();
    const program = readProgram(conn);
    const cfg: any = await (program.account as any).annaumixyzConfig.fetch(CONFIG_PDA);
    return {
...
export async function fetchRecentBlocks(limit = 12): Promise<MinedBlock[]> {
  try {
    ...
    for (const s of sigs) {
      ...
      const isMined = logs.some((l) => l.includes("annaumixyz: mined block"));
      if (!isMined) continue;

      // Parse height + winner from log
      const mineLog = logs.find((l) => l.includes("annaumixyz: mined block"));
