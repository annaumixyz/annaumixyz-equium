...
const ANX_DECIMALS = 6;
const SOL_FEE_BUFFER_LAMPORTS = 5_000; // typical signature fee
const ANX_RENT_BUFFER_LAMPORTS = 0.0021 * LAMPORTS_PER_SOL; // ATA creation if recipient is new
...
export function SendModal({
  ...
  const max =
    token === "SOL"
      ...
      : Number(eqmBase) / 10 ** ANX_DECIMALS;
  ...
  const setMax = () => {
    ...
    else setAmount(max.toFixed(ANX_DECIMALS));
  };
    ...
    try {
      ...
      if (token === "SOL") {
        ...
        const baseUnits = BigInt(
          Math.round(amountNum * 10 ** ANX_DECIMALS)
        );
        tx.add(
          createTransferCheckedInstruction(
            sourceAta,
          ...
          <span>
            ...
            <span className="font-bold text-[var(--color-fg)]">
              {token === "SOL"
                ...
                : `${(Number(eqmBase) / 10 ** ANX_DECIMALS).toFixed(ANX_DECIMALS)} ANX`}
            </span>
          </span>
          <button
            onClick={setMax}
