...
export function ShareCardModal({ onClose, pubkey, stats }: Props) {
  ...
  const download = async () => {
    ...
    try {
      ...
      a.href = dataUrl;
      a.download = `annaumixyz-mining-${Date.now()}.png`;
  ...
  const tweet = () => {
    const text = encodeURIComponent(
      `Mining $ANX on @Annaumixyz 🚀\n\n` +
        `⛏ Hashrate: ${formatHashrate(stats.hashrate)}\n` +
        `📦 Blocks: ${stats.blocks}\n` +
        `💰 Earned: ${formatEqm(stats.earnedBase)} ANX\n\n` +
        `Mine in your browser — fair-launched, CPU-only, on Solana.`
    );
    window.open(
      `https://twitter.com/intent/tweet?text=${text}&url=https://annaumixyz.xyz/mine`,
      "_blank",
      "noopener,noreferrer"
    );
  };
