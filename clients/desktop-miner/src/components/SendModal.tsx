...
import { ANX_DECIMALS, formatEqm, formatSol } from "../lib/format";
...
export default function SendModal({
  ...
  const setMax = () => {
    if (max <= 0) return;
    setAmount(token === "SOL" ? max.toFixed(6) : max.toFixed(ANX_DECIMALS));
  };
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
          ...
          <span className="mono dim" style={{ fontSize: 11 }}>
            ...
            <span style={{ color: "var(--ink)", fontWeight: 600 }}>
              {token === "SOL"
                ...
                : `${formatEqm(eqmBase, ANX_DECIMALS)} ANX`}
            </span>
          </span>
          <button
            className="copybtn"
        ...
        {token === "ANX" && (
          <p className="dim" style={{ fontSize: 11.5, marginTop: 10 }}>
            If the recipient has never held ANX, a small SOL fee (~0.002 SOL)
