...
The frontend depends on `annaumixyz-wasm` from `wasm/pkg/`, produced by
...
```bash
cp ../../../target/idl/annaumixyz.json app/src/idl.json
```

## How it works

1. On wallet connect, the app fetches the `AnnaumixyzConfig` PDA every 5 s
