...
const IDL_PATH = path.resolve(process.cwd(), "target/idl/annaumixyz.json");
const SOLVER_BIN = path.resolve(
  process.cwd(),
  "target/release/examples/solve_block"
);

const CONFIG_SEED = Buffer.from("annaumixyz-config");
const VAULT_SEED = Buffer.from("annaumixyz-vault");
const N = 96;
const K = 5;
...
describe("annaumixyz pre-mint architecture", () => {
  ...
  it("initialize: sets up config + empty vault, references existing mint", async () => {
    await program.methods
      .initialize({
        ...
        equihashN: N,
        equihashK: K,
        metadataUri: "https://annaumixyz.example/v1.json",
      })
      .accounts({
    ...
    const cfg = await program.account.annaumixyzConfig.fetch(configPda);
  ...
  it("mine: pays the block reward into the miner's ATA", async () => {
    const cfgBefore = await program.account.annaumixyzConfig.fetch(configPda);
    ...
    const cfgAfter = await program.account.annaumixyzConfig.fetch(configPda);
    const adminAfter = await getAccount(connection, adminAta, undefined, TOKEN_PROGRAM_ID);
    const vaultAfter = await getAccount(connection, vaultPda, undefined, TOKEN_PROGRAM_ID);
