...
pub fn read_recent_slot_hash(account: &AccountInfo) -> Result<[u8; 32]> {
    let data = account
        .try_borrow_data()
        .map_err(|_| error!(AnnaumixyzError::BadSlotHashes))?;

    if data.len() < 8 + 8 + 32 {
        return Err(error!(AnnaumixyzError::BadSlotHashes));
    }
...
pub fn verify_submission(
    ...
    verify::verify(n, k, &input, nonce, soln_indices, target).map_err(|e| match e {
        verify::VerifyError::InvalidEquihash => error!(AnnaumixyzError::InvalidEquihash),
        verify::VerifyError::AboveTarget => error!(AnnaumixyzError::AboveTarget),
    })
}
