//! Annaumixyz ($ANX) — on-chain CPU mining program.
//!
...
use crate::state::{AnnaumixyzConfig, CONFIG_SEED, VAULT_SEED};

declare_id!("ZKGMUfxiRCXFPnqz9zgqAnuqJy15jk7fKbR4o6FuANX");
...
pub const EXPECTED_ADMIN: Pubkey = pubkey!("AgbSti5LyTfYHVytBhNP8HHz3Ko7bZfSvnsm9cJAANX");
...
pub struct Initialize<'info> {
    #[account(mut, address = EXPECTED_ADMIN @ AnnaumixyzError::NotAdmin)]
    pub admin: Signer<'info>,

    #[account(
        init,
        payer = admin,
        space = 8 + AnnaumixyzConfig::INIT_SPACE,
        seeds = [CONFIG_SEED],
        bump,
    )]
    pub config: Account<'info, AnnaumixyzConfig>,
...
pub struct FundVault<'info> {
    ...
    #[account(
        ...
        has_one = admin @ AnnaumixyzError::NotAdmin,
        has_one = mint @ AnnaumixyzError::WrongMint,
        has_one = mineable_vault @ AnnaumixyzError::WrongVault,
    )]
    pub config: Account<'info, AnnaumixyzConfig>,
    ...
    #[account(
        mut,
        constraint = source.mint == mint.key() @ AnnaumixyzError::SourceMintMismatch,
    )]
