...
pub fn handler(
    ...
    require!(cfg.mining_open, AnnaumixyzError::MiningNotOpen);
    ...
    require!(reward > 0, AnnaumixyzError::RewardZeroed);

    let new_cumulative = cfg
        .cumulative_mined
        .checked_add(reward)
        .ok_or_else(|| error!(AnnaumixyzError::ScheduleOverflow))?;
    require!(new_cumulative <= MINEABLE_BASE, AnnaumixyzError::SupplyExhausted);
    ...
    msg!(
        "annaumixyz: mined block {} by {} for {} base units",
        new_height - 1,
        miner,
        reward
    );
