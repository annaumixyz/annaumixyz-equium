//! Sets up the Annaumixyz config PDA + creates an empty mineable vault PDA.
//!
...
pub fn handler(ctx: Context<Initialize>, args: InitializeArgs) -> Result<()> {
    ...
    msg!(
        "annaumixyz: initialized at slot {} (mint {}, vault {}); mining_open=false",
        cfg.genesis_slot,
        mint_key,
        vault_key
    );
...
#[event]
pub struct AnnaumixyzInitialized {
    pub admin: Pubkey,
    pub config: Pubkey,
    pub mint: Pubkey,
